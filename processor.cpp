...
RUNNER_ERROR Run(int code[], size_t size)
{
    ...
    while (running)
    {
        switch (code[spu.ip])
        {
            ...
            case (PUSHR):
                {
                    Stack_Push(spu.stk, spu.registers[spu.code[spu.ip + 1]]);
                    spu.ip += 2;
                    break;
                }
            ...
            default:
                {
                    printf(ANSI_RED "UNKNOWN CMD=%d in %s:%d(%s)" ANSI_RESET_COLOR, spu.code[spu.ip], __FILE__, __LINE__, __FUNCTION__);
                    abort();
                    break;
                }
        }
